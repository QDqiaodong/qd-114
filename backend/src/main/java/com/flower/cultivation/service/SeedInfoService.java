...
public class SeedInfoService {
    ...
    public SeedInfo save(SeedInfo seedInfo) {
        if (seedInfo.getInitialQuantity() == null || seedInfo.getInitialQuantity() <= 0) {
            throw new RuntimeException("初始数量必须大于0");
        }
        if (seedInfo.getId() == null && seedInfo.getRemainingQuantity() == null) {
            seedInfo.setRemainingQuantity(seedInfo.getInitialQuantity());
        }
        if (seedInfo.getRemainingQuantity() == null || seedInfo.getRemainingQuantity() < 0) {
            throw new RuntimeException("剩余数量不能为负数");
        }
        if (seedInfo.getRemainingQuantity() > seedInfo.getInitialQuantity()) {
        ...
        if (seedInfo.getId() != null) {
            ...
            if (seedInfo.getInitialQuantity() - seedInfo.getRemainingQuantity() < totalSown) {
